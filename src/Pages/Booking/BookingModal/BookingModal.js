...
const BookingModal = ({openBooking, handleBookingClose, timeSlot, date, setBookingSuccess}) => {
  ...
  const [timeSlotInfo, settimeSlotInfo] = useState(initialInfo);


  const handleOnBlur = e => {
    const field = e.target.name;
    const value = e.target.value;
    const newInfo = { ...timeSlotInfo };
    newInfo[field] = value;
    settimeSlotInfo(newInfo);
}
  ...
  const handleBookingSubmit = e =>{
    ...
    fetch('https://arcane-springs-99737.herokuapp.com/appointments', {
      method: 'POST',
      headers: {
